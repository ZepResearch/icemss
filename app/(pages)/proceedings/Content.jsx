...
const PB_URL = "https://icemss.pockethost.io"
    ...
    useEffect(() => {
        async function fetchProceedings() {
            try {
                ...
                const records = await pb.collection("proceedings").getFullList({
                    sort: "created",
                })
    ...
    return (
        <section className="relative min-h-screen bg-white overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-blue-50 rounded-full -translate-x-1/2 -translate-y-1/2 opacity-60" />
                <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-blue-50 rounded-full translate-x-1/3 translate-y-1/3 opacity-50" />
                <div className="absolute top-1/2 left-1/2 w-[300px] h-[300px] bg-sky-50 rounded-full -translate-x-1/2 -translate-y-1/2 opacity-40" />
            </div>
...
function ProceedingCard({ item, index }) {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-blue-100/50 transition-shadow duration-300 overflow-hidden flex flex-col"
        >
            {/* Cover Image */}
            <div className="relative w-full aspect-[3/4] bg-gradient-to-br from-blue-50 to-sky-50 overflow-hidden">
                {coverUrl ? (
                    <Image
                        src={coverUrl}
            ...
            <div className="flex flex-col flex-1 p-6">
                ...
                {item.conference_acronym && (
                    <span className="self-start inline-block bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full mb-3 uppercase tracking-wider">
                        {item.conference_acronym}
                    </span>
                )}
                ...
                {pdfUrl && (
                    <a
                        ...
                        className="inline-flex items-center justify-center gap-2 w-full bg-gradient-to-r from-blue-600 to-sky-500 hover:from-blue-700 hover:to-sky-600 text-white text-sm font-medium py-3 px-4 rounded-xl transition-all duration-200 shadow-sm hover:shadow-md hover:shadow-blue-200"
                    >
